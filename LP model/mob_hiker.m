...
mob = [0 3 6 14];          % mobilities for hiker LPT from LPB book. These define a cdf for the mobility 

% compute pdf from cdf
bins = 0.5:1:14;                  % define bins to ninterpolate cdf
cdf = interp1(mob, p, bins);      % interpolate cdf at bins
% pdf = diff(cdf)/sum(diff(cdf));  % differentiate cdf to get pdf
% bins_pdf = 0.5:1:13.5;          % define bins for pdf (centers from cdf bins) 
...
% The best lambda (R^2=0.972) is below. 

a = 4.159;            % parameter of best fit Weibull distribution for pdf
b = 0.7183;
