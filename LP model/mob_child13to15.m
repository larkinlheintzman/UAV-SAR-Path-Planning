function Mob = mob_child_13to15(P,R)
...
mob = [0.1 3.5 5 12];          % mobilities for child (ages 13-15) LPT from LPB book. These define a cdf for the mobility 

% compute pdf from cdf
bins = 0.1:1:12;                  % define bins to ninterpolate cdf
cdf = interp1(mob,p, bins);      % interpolate cdf at bins
% pdf = diff(cdf)/sum(diff(cdf));  % differentiate cdf to get pdf
% bins_pdf = 0.6:1:11.5;          % define bins for pdf (centers from cdf bins) 
...
% The best lambda (R^2=0.9104) is below. 

a = 3.99;            % parameter of best fit Weibull distribution for pdf
b = 0.8046;
