...
mob = [3 7 10 19];          % mobilities for worker LPT from LPB book. These define a cdf for the mobility 

% compute pdf from cdf
bins = 3:1:19;                  % define bins to ninterpolate cdf
cdf = interp1(mob,p, bins);      % interpolate cdf at bins
% pdf = diff(cdf)/sum(diff(cdf));  % differentiate cdf to get pdf
% bins_pdf = 3.5:1:18.5;          % define bins for pdf (centers from cdf bins) 
...
% The best lambda (R^2=0.9923) is below. 

a = 8.613;            % parameter of best fit Weibull distribution for pdf
b = 1.316;
