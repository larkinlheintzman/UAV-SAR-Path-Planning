...
mob = [0 4 8 26];          % mobilities for hunter LPT from LPB book. These define a cdf for the mobility 

% compute pdf from cdf
bins = 0:1.5:26;                  % define bins to ninterpolate cdf
cdf = interp1(mob, p, bins);      % interpolate cdf at bins
% pdf = diff(cdf)/sum(diff(cdf));  % differentiate cdf to get pdf
% bins_pdf = 0.75:1.5:25.25;          % define bins for pdf (centers from cdf bins) 
...
% The best lambda (R^2=0.9078) is below. 

a = 6.062;            % parameter of best fit Weibull distribution for pdf
b = 0.6723;
