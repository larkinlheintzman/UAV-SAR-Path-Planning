function Mob = mob_child1to6(P,R)
...
mob = [0 1 2 4.3];          % mobilities for child (ages 1-6) LPT from LPB book. These define a cdf for the mobility 

% compute pdf from cdf
% bins = 0.1:0.5:4.1;                  % define bins to ninterpolate cdf
bins = 0:0.3:4.3;                  % define bins to ninterpolate cdf

cdf = interp1(mob,p, bins);      % interpolate cdf at bins
% pdf = diff(cdf)/sum(diff(cdf));  % differentiate cdf to get pdf
% bins_pdf = 0.25:0.5:3.75;          % define bins for pdf (centers from cdf bins) 
...
% The best lambda (R^2=0.9446) is below. 

a = 1.317;            % parameter of best fit Weibull distribution for pdf
b = 0.6732;
