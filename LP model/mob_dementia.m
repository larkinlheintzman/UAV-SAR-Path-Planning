...
mob = [0 0.25 3.8 18];      % mobilities for people with dementia from LPB book. These define a cdf for the mobility 

% compute pdf from cdf
bins = 0:1:18;                  % define bins to ninterpolate cdf
cdf = interp1(mob,p, bins);     % interpolate cdf at bins
% pdf = diff(cdf)/sum(diff(cdf)); % differentiate cdf to get pdf
% bins_pdf = 0.5:1:17.5;          % define bins for pdf (centers from cdf bins) 
...
% The best lambda (R^2=0.8637) is below. 

a = 1.973;            % parameter of best fit Weibull distribution for pdf
b = 0.4039;
