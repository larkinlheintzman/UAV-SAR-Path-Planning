...
mob = [0 2 7 21];          % mobilities for mountain biker LPT from LPB book. These define a cdf for the mobility 

% compute pdf from cdf
bins = 0.5:1:21;                  % define bins to ninterpolate cdf
cdf = interp1(mob,p, bins);      % interpolate cdf at bins
% pdf = diff(cdf)/sum(diff(cdf));  % differentiate cdf to get pdf
% bins_pdf = 0.5:1:20.5;          % define bins for pdf (centers from cdf bins) 
...
% The best lambda (R^2=0.9897) is below. 

a = 4.169;            % parameter of best fit Weibull distribution for pdf
b = 0.547;
